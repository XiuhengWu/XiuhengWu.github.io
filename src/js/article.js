...
function getReadingProgress() {
	...
	if (!current_heading) { current_heading = titles[titles.length - 1].elem.id; }
...
window.addEventListener('load', (event) => {
	...
	if (localStorage.getItem('dynamic_background')) { config['dynamic_background'] = localStorage.getItem('dynamic_background') == 'true' }
	// appearance
	...
	titles.forEach(title => {
		catalogs.forEach(elem => {
			elem.innerHTML += `
			<li style='padding-left: ${title.level * 1}rem'>
				<a class="t${title.level}" href="javascript:void(0);" onclick="location.replace('#${title.elem.id}');" data-match="${title.elem.id}">${title.elem.innerHTML}</a>
			</li>
			`
		});
		...
		button.addEventListener('click', function () {
			checkFileExists(pdf_path, function(exists) {
				if (exists) {
					...
					alert({
						headline: "Error",
						description: "There is no PDF for this article",
						confirmText: "OK",
					});
				}
			});
	...
	if (! config['dynamic_background']) { particles.pauseAnimation(); }
});
