...
function replaceAllFractions(str) {
	...
	function extractFractions(expr) {
		...
		let match;
		while ((match = fractionRegex.exec(expr)) !== null) { // Recursively extract fractions from the numerator and denominator
...
function replaceAllBrackets(str, max_depths) {
	...
	while (str[i]) {
		if (str[i] == "(") {
			str = str.slice(0, i) + `<div class="bracket s${
				max_depths - current_depth
			}"></div>` + str.slice(i + 1);
			current_depth++;
		} else if (str[i] == ")") {
			current_depth--;
			str = str.slice(0, i) + `<div class="bracket right s${
				max_depths - current_depth
			}"></div>` + str.slice(i + 1);
		}
		i++;
...
function render(source, result_elem) {
	...
	source = source.replaceAll(/\b(a)\b/g, '<img src="./kobe/2.webp">').replaceAll(/\b(b)\b/g, '<img src="./kobe/4.webp">').replaceAll(/\b(c)\b/g, '<img src="./kobe/8.webp">').replaceAll(/\b(d)\b/g, '<img src="./kobe/24.webp">').replaceAll(/\b(e)\b/g, '<img src="./kobe/24.webp">');
	result_elem.style.setProperty("--f-size", `${
		clamp(max_depths * 1, 2, 5)
	}rem`);
	return source;
}
...
function copyExpressionToClickBoard(simplified_expr) {
	...
	if (simplified_expr) {
		navigator.clipboard.writeText(simplified_expr.replaceAll("a", 2).replaceAll("b", 4).replaceAll("c", 8).replaceAll("d", 24).replaceAll("e", 24).replaceAll("^", "**")).then(() => {
			document.querySelector("button.copy > .text-content").innerText = language_pack["clipboard_success"][current_lan];
		}).catch((err) => {
			document.querySelector("button.copy > .text-content").innerText = language_pack["clipboard_err_2"][current_lan];
		});
	}
}
...
const language_pack = {
	clipboard_err_1: {
		zh: "孩子，你的浏览器不支持Clipboard API",
		de: "Ihr Browser unterstützt die Clipboard-API nicht."
	},
	clipboard_err_2: {
		zh: "Man！无法访问剪贴板！",
		de: "Zugriff auf die Clipboard nicht möglich!"
	},
	clipboard_success: {
		zh: "已复制",
		de: "Kopiert"
	...
	invalid_input_warn: {
		zh: "请输入一个整数！",
		de: "Bitte geben Sie eine ganze Zahl ein!"
	},
	copy: {
		zh: "复制曼巴表达式",
		de: "Mamba-Ausdruck Kopieren"
	},
	prepared: {
		zh: "孩子，你可以开始输入了",
		de: "Sie können jetzt ein Zahl eingeben"
	}
};
...
document.addEventListener("DOMContentLoaded", (event) => {
	...
	document.querySelector("#go").addEventListener("click", function () {
		...
		if (input_value == "" || input_value.includes(".")) {
            ...
            <p>${
				language_pack["invalid_input_warn"][current_lan]
			}</p>
            `;
			return;
		}
...
result_elem.innerHTML = `<p class="info">${
	language_pack["prepared"][current_lan]
}</p>`;});
